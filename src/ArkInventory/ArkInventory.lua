...
function ArkInventory.Frame_Item_Update_Texture( frame )
	...
	ArkInventory.Frame_Item_Update_Attunement( frame )
end
...
function ArkInventory.Frame_Item_Update_Attunement( frame )
	
	if ArkInventory.ValidFrame( frame, true ) == false or (not SynastriaCoreLib) then
		return
	end

	local framename = frame:GetName( )
	
	local obj_name = "ArkAttunementProgress"
	local obj = _G[string.format( "%s%s", framename, obj_name )]
	assert( obj, string.format( "xml element '%s' is missing the sub element %s", framename, obj_name ) )
